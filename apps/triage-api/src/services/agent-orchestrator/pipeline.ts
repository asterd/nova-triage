...
export const runOrchestrationPipeline = async (
    ...
    const normStr = await invokeNovaLite(INTAKE_NORMALIZER_PROMPT, JSON.stringify({ raw_input: rawInput, demographics, pain_score: painScore, onset }));
    const normOutput = JSON.parse(cleanJSON(normStr));

    // 2. Symptom Structurer (Nova Lite)
    const structStr = await invokeNovaLite(SYMPTOM_STRUCTURER_PROMPT, JSON.stringify(normOutput));
    ...
    if (attachments && attachments.length > 0) {
        ...
        const mmStr = await invokeNovaMultimodal(
    ...
    const riskStr = await invokeNovaLite(RISK_CLASSIFIER_PROMPT, JSON.stringify(riskContext));
    const riskOutput = JSON.parse(cleanJSON(riskStr));

    // 5. Differential Clusters (Nova Pro)
    const clusterStr = await invokeNovaPro(DIFFERENTIAL_CLUSTER_PROMPT, JSON.stringify(structOutput));
    ...
    const handoffStr = await invokeNovaPro(HANDOFF_COMPOSER_PROMPT, JSON.stringify(handoffContext));
    const handoffOutput = JSON.parse(cleanJSON(handoffStr));

    // 7. Patient Explanation (Nova Lite)
    const patientStr = await invokeNovaLite(PATIENT_EXPLANATION_PROMPT, JSON.stringify(handoffContext));
