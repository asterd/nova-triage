...
exports.evaluateSafetyRules = exports.getHigherUrgency = void 0;
const urgencyRank = {
    minimal: 0,
    low: 1,
    moderate: 2,
...
const asLowerText = (value) => (typeof value === 'string' ? value.toLowerCase() : '');
const asStringArray = (value) => (Array.isArray(value) ? value.filter((item) => typeof item === 'string') : []);
const hasAny = (haystack, terms) => terms.some((term) => haystack.includes(term));
const hasTermsInCollections = (collections, terms) => collections.some((entry) => hasAny(entry.toLowerCase(), terms));
const mergeMinimumUrgency = (current, next) => !current || urgencyRank[next] > urgencyRank[current] ? next : current;
const getHigherUrgency = (left, right) => urgencyRank[left] >= urgencyRank[right] ? left : right;
exports.getHigherUrgency = getHigherUrgency;
const evaluateSafetyRules = (input) => {
    ...
    const trigger = (ruleId, urgency, redFlag, note) => {
    ...
    if ((hasAny(searchable, ['chest pain', 'dolore toracico', 'tight chest', 'chest pressure']) &&
        ...
        trigger('severe_chest_pain_sudden_onset', 'critical', 'Severe chest pain with sudden onset', 'Deterministic override applied for severe chest pain with sudden onset.');
    ...
    if (hasAny(searchable, ['shortness of breath', 'breathless', 'dispnea', 'can’t breathe', 'cannot breathe', 'fatica respiratoria'])) {
        ...
        if (severeResp) {
            trigger('severe_dyspnea', 'critical', 'Severe shortness of breath', 'Deterministic override applied for severe respiratory distress.');
        }
        else {
            trigger('dyspnea_requires_urgent_assessment', 'high', 'Shortness of breath requires urgent assessment', 'Deterministic escalation applied for respiratory symptoms.');
        }
    }
    if (hasAny(searchable, ['stroke', 'facial droop', 'slurred speech', 'weakness', 'numbness', 'one-sided', 'hemiparesis', 'confusion improvvisa'])) {
        trigger('possible_stroke', 'critical', 'Stroke-compatible neurologic symptoms', 'Deterministic override applied for possible stroke pattern.');
    ...
    if (hasAny(searchable, ['altered mental status', 'confused', 'not responding', 'lethargic', 'difficult to wake', 'confusione', 'disorientato'])) {
        trigger('altered_mental_status', 'critical', 'Altered mental status', 'Deterministic override applied for altered mental status.');
    }
    if (hasAny(searchable, ['anaphylaxis', 'swollen tongue', 'swollen lips', 'hives', 'urticaria', 'allergic reaction', 'reazione allergica'])) {
        trigger('suspected_anaphylaxis', 'critical', 'Possible anaphylaxis', 'Deterministic override applied for suspected anaphylaxis.');
    }
    if (hasAny(searchable, ['bleeding heavily', 'heavy bleeding', 'emorragia', 'hemorrhage', 'blood everywhere', 'vomiting blood', 'coughing blood'])) {
        trigger('major_hemorrhage', 'critical', 'Major bleeding reported', 'Deterministic override applied for major hemorrhage.');
    }
    if (hasAny(searchable, ['fever', 'febbre', '39', '40', '104']) && (ageBand === '0-1' || frailty)) {
        trigger('high_fever_fragile_patient', 'high', 'High fever in infant or frail patient', 'Deterministic escalation applied for fever in fragile patient.');
